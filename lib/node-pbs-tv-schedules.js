...
PBSTvSchedules.prototype.standard_http_get = function (options) {
    ...
    request(options, function(err, res, obj){
        ...
        try {
            ...
            logger.error('PBSTvSchedules.standard_http_get', e);
            deferred.reject(e);
        }
...
PBSTvSchedules.prototype.get_upcoming_by_callsign_program_id = function (program_id, callsign) {
    ...
    request(options, function(err, res, obj){
        if (err) {
            logger.error('PBSTvSchedules.get_upcoming_by_callsign_program_id', err);
            deferred.reject(err);
        }
        var obj_parsed;
        if (!res || res.statusCode !== 200) {
        ...
        try {
            ...
            logger.error("PBSTvSchedules.get_upcoming_by_callsign_program_id parse", e);
...
PBSTvSchedules.prototype.get_callsigns_by_zip = function (zip) {
    ...
    request(options, function(err, res, obj){
        if (err) {
            logger.error('PBSTvSchedules.get_callsigns_by_zip', err);
            deferred.reject(err);
        }
        var callsigns = [],
            data;
        ...
        try {
            ...
            logger.error('PBSTvSchedules.get_callsigns_by_zip', e);
            deferred.reject(e);
        }
